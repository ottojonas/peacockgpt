  ...
  const handleConversationClick = async (key: string) => {
    ...
    if (selectedConversation) {
      ...
      try {
        ...
        const updatedConversationResponse = await axios.get(
          `/api/conversations?key=${key}`
        );
        const updatedConversation = updatedConversationResponse.data;
        const date = new Date(updatedConversation.date);
        updatedConversation.date = isNaN(date.getTime())
          ? new Date().toISOString()
          : date.toISOString();

        setConversations((prevConversations) =>
          prevConversations.map((conversation) =>
            conversation.key === key ? updatedConversation : conversation
          )
        );
      } catch (error) {
...
function Item({
  ...
  const formattedDate = isValidDate(item.date)
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col items-end">
            <span className="pl-2 shrink-0">{formattedDate}</span>
